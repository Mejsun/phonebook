...
import { ContactsWrapper, Options, Header, ContactForm, Subheader, Thead, Tr, Td} from '../styles/StyledComps';
...
function Contacts() {
  ...
  const contactInfo = {contactName: '', company: '', primaryEmailAddress: '', mobilephone:'', homephone:'', workphone:'' } //default contact info object
  ...
  useEffect(() => { //populate the contacts with the data from api on page render
  ...
  function handleAddFormChange(e){ //add new contact tracking the input values
    e.preventDefault();
  ...
  function handleAddFormSubmit (e){ //submit the new contact data
    e.preventDefault();
    const newContact = { //new contact object values
      id: nanoid(), 
  ...
  function handleDelete(id){ //delete contact based on the id
    const removeContact = contacts.filter((contact)=>{return contact.id !== id}) //filter the contacts array to all except the selected contact
  ...
  function handleEdit(e){ //edit the contact data
    e.preventDefault()
    const fieldName = e.target.getAttribute('name'); //track the inputs by the name attribute
    const fieldValue = e.target.value;
  ...
  function editId (e, contact){ //get the correct contact to be edited
    e.preventDefault()
    setEditContactId(contact.id)
    const formValues = { //populate the edit input with contacts existing data
  ...
  function editSubmit (e){ //submit the changes to contact data
    e.preventDefault()
    const edited = { //new contact data info
      id: editContactId,
  ...
  return (
    ...
    {getToken ? ( 
      ...
      <ContactForm onSubmit={editSubmit}>
        <table>
          <Thead>
            <Tr>{headers.map((header,i) => (<Td key={i}><Subheader>{header}</Subheader></Td>))}</Tr>
          </Thead>
          <tbody>
